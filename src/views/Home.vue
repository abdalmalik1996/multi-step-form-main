...
<template>
  <VContainer
    ...
    :class="{ 'h-100': !this.display.smAndDown }"
  >
    <v-sheet class="rounded-xl" :class="{ 'w-100': display.smAndDown }">
      <VCard class="d-block d-md-flex rounded-xl" variant="flat" max-width="986" width="100%">
        <AppMain :step="step" />
        <v-card
          :class="{ disctop: !display.smAndDown, mobile: display.smAndDown }"
          variant="flat"
          class="d-flex flex-column px-md-13 pt-md-10 pb-md-5"
        >
          <v-card-item :class="{ 'bg-white rounded-lg ': this.display.smAndDown }">
            <keep-alive>
              <transition name="fade" mode="out-in">
                <component :is="select[step]"> </component>
              </transition>
            </keep-alive>
          </v-card-item>
          ...
          <VCardActions class="pa-3" v-if="!confirm" :class="{ 'bg-white': display.smAndDown }">
            <v-hover>
              <template v-slot:default="{isHovering,props}">
                <v-btn v-bind="props" :class="isHovering ? 'text-blue-darken-4' : ''" class=" text-caption " variant="plain"  v-if=" step !==0  " @click="GoBack">Go Back</v-btn>
              </template>
            </v-hover>
            <VSpacer />
            <v-hover>
